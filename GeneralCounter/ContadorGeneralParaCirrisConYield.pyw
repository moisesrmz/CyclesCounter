...
class FileModifiedHandler(FileSystemEventHandler):
    def __init__(self, label, cycle_label, fail_label, yield_label, canvas, frame_color):
        ...
        ########################################### nuevas para bloqueo por yield bajo 
        os.makedirs(r"C:\Users\Public\Documents\Cirris\status", exist_ok=True)
    ...
    def update_yield(self):
        ...
        # Leer estado actual
        try:
            with open(r"C:\Users\Public\Documents\Cirris\status\yield_status.txt", "r") as f:
                content = f.read()
                if content.startswith("OK|"):
                    match = re.match(r"OK\|[\d\-:\s]+\|([\d\.]+)", content)
                    if match:
                        saved_yield = float(match.group(1))
        except Exception as e:
            print(f"⚠️ Error leyendo estado anterior: {e}")

        # Evaluar condición de bloqueo
        if self.yield_value < 90:
            if not self.manual_unlock:
                lock_status = "LOCKED"
            elif saved_yield is not None and self.yield_value < saved_yield - 5:
                print(f"🔒 Yield cayó más de 5% desde desbloqueo manual ({saved_yield:.2f} → {self.yield_value:.2f})")
                lock_status = "LOCKED"
                self.manual_unlock = False
        ...
        # Debug directo para validar
        print(f"[DEBUG] manual_unlock={self.manual_unlock}, yield_value={self.yield_value:.2f}, saved_yield={saved_yield}, lock_status={lock_status}")

        # Guardar estado
        try:
            with open(r"C:\Users\Public\Documents\Cirris\status\yield_status.txt", "w") as status_file:
                if lock_status == "OK":
                    status_file.write(f"OK|{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}|{self.yield_value:.2f}")
                else:
                    status_file.write("LOCKED")
        except Exception as e:
            print(f"⚠️ Error al escribir estado de yield: {e}")
