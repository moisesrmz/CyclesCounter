...
class FileModifiedHandler(FileSystemEventHandler):
    ...
    def reset_counters(self):
        ...
        safe_ui(root.title, f"Corrida new Actual - Turno {turno} --First Pass Yield--")
...
def update_window_title():
    ...
    if current_test_state:
        root.title(
            f"Corrida new Actual - Turno {turno} --First Pass Yield--                  {current_test_state}"
        )
    else:
        root.title(
            f"Corrida new Actual - Turno {turno} --First Pass Yield--"
        )
...
def main():
    ...
    root.title(f"Corrida new Actual - Turno {turno} --First Pass Yield--")
...
def toggle_vbs_script(event=None):
    ...
    script_path = r"\\mlxgumvwfile01\Departamentos\Fakra\Pruebas\CyclesCounter\CyclesCounter.vbs"

    # iniciar script si no está corriendo, o detenerlo si ya está activo
    if vbs_process is None or vbs_process.poll() is not None:
