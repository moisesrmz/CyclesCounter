...
class FileModifiedHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if not event.src_path.lower().endswith(".prn"):
            print(f"Archivo ignorado: {event.src_path}")  # Debug: Ignorar archivos no PRN
            return
        ...
        if file_path.lower().endswith(".prn"):  
            ...
            for attempt in range(attempts):
                try:
                    ...
                    #time.sleep(0.2)  # Esperar 200 ms para asegurar que el archivo esté listo
                    with open(file_path, "r", encoding="utf-8") as file:
                        # ✅ Verificar primera línea para detectar falla
                        first_line = file.readline().strip()
                        print(f"Primera línea leída: '{first_line}'")

                        if "^XA" in first_line:
                            self.fail_count += 1
                            self.fail_label.config(text=f"{self.fail_count} ")                  
                            print(f"Falla detectada en {file_path} (Total: {self.fail_count})")

                        # ✅ Buscar número de parte después de 'FD'
                        file.seek(0)
                        for line in file:
                            match = re.search(r"FD([^\s\^]+)", line)
                            if match:
                                part_number = match.group(1)
                                print(f"🔧 Numero de parte: {part_number}")
                                break
                    break  # ✅ Archivo leído exitosamente, salimos del bucle
                    ...
                    break  # 🛑 Detener reintentos en errores distintos a PermissionError
    ...
    def draw_gauge(self):
        ...
        if self.yield_value <= 80:
            ...
            bar_color = (255/255, 215/255, 64/255, 0.9)  # Amarillo cálido
        else:
            bar_color = (114/255, 196/255, 149/255, 0.9)  # Pastel verde
        ax.add_patch(Wedge((0, 0), 1, 0, 180, facecolor="#B0B0B0", edgecolor="none", lw=1)) ## gris mas oscuro
